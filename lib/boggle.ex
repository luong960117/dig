defmodule Boggle do
    ...
    def wordBoggle(board, words, word_map, not_exist_word) do
        if length(words) == 0 do
            word_map
        else
            word = hd(words)
            if (not_exist_word == "") do
                res = search_a_word(board, word)
                if res != false do
                    new_map = Map.put(word_map, to_string(word), res)
                    not_exist_word = ""
                    wordBoggle(board, tl(words), new_map, not_exist_word)
                else
                    not_exist_word = word
                    wordBoggle(board, tl(words), word_map, not_exist_word)
                end
            else
                if !String.contains?(word, not_exist_word) do
                    res = search_a_word(board, word)
                    if res != false do
                        new_map = Map.put(word_map, to_string(word), res)
                        not_exist_word = ""
